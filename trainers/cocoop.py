...
class PromptLearner(nn.Module):
    ...
    def forward(self, im_features):
        ...
        raw_features = bias / bias.norm(dim=-1, keepdim=True)
        self.raw_image_features_store.append(raw_features.cpu().detach())
        ...
        return prompts, ctx_shifted, raw_features
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.raw_image_features_store = []  # Stores raw image features (before projection)
        self.processed_image_features_store = []  # Stores processed image features
        self.label_store = []
    ...
    def save_features(self, raw_features, processed_features, labels):
        """Save raw and processed image features along with labels."""
        save_dir = "/apps/hari/CoOp/experiments"
        os.makedirs(save_dir, exist_ok=True)

        # Save raw features
        torch.save(raw_features, os.path.join(save_dir, "raw_image_features.pth"))
    
        # Save processed features
        torch.save(processed_features, os.path.join(save_dir, "processed_image_features.pth"))

        # Save labels
        torch.save(labels, os.path.join(save_dir, "labels.pth"))
    ...
    def forward(self, image, label, aug_images):
        ...
        raw_features = image_features / image_features.norm(dim=-1, keepdim=True)
        ...
        #self.save_features(raw_features.cpu().detach(), image_features.cpu().detach(), label.cpu().detach())
        self.raw_image_features_store.append(raw_features.cpu().detach())  # Before projection
        #self.processed_image_features_store.append(image_features.cpu().detach())  # After processing
        self.label_store.append(label.cpu().detach())  # Store labels        
        self.processed_image_features_store.append(processed_features.cpu().detach())  # After processing
        ...
        prompts, ctx_shifted,raw_features = self.prompt_learner(image_features)
        _, ctx_shifted1,_ = self.prompt_learner(image_features1)
        ...
        return logits, ctx_shifted, label, raw_features, processed_features
...
class CoCoOp(TrainerX):
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            # Get features
            logits, ctx_shifted, label, raw_features, processed_features = self.model_inference(input, label)

            # Append to storage
            all_raw_features.append(raw_features.cpu().detach())
            all_processed_features.append(processed_features.cpu().detach())
            all_labels.append(label.cpu().detach())

            self.evaluator.process(logits, label)

        # Concatenate all features after testing
        all_raw_features = torch.cat(all_raw_features)
        all_processed_features = torch.cat(all_processed_features)
        ...
        torch.save(all_raw_features, os.path.join(save_dir, "raw_image_features.pth"))
        torch.save(all_processed_features, os.path.join(save_dir, "processed_image_features.pth"))
        torch.save(all_labels, os.path.join(save_dir, "labels.pth"))

        print(f"Saved {all_raw_features.shape[0]} test samples.")
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            input, label = self.parse_batch_test(batch)
            logits, ctx_shifted, label,_,_ = self.model_inference(input, label, aug_images=None)
            self.evaluator.process(logits, label)
