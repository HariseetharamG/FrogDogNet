...
class KalmanAdaptiveFourier(nn.Module):
    def __init__(self, init_cutoff=75, process_var=1e-3, measure_var=1e-2):
        ...
        # State estimate (scalar)
        self.register_buffer("cutoff_est", torch.tensor(float(init_cutoff)))

        # State covariance (scalar)
        self.register_buffer("P", torch.tensor(1.0))
    ...
    def forward(self, x):
        ...
        # -------------------------
        # Step 2: Spectral Energy
        # -------------------------
        spectral_energy = fft_x.abs() ** 2
        cumulative_energy = torch.cumsum(
            spectral_energy, dim=-1
        ) / torch.sum(spectral_energy, dim=-1, keepdim=True)

        # -------------------------
        # Step 3: Measure cutoff (95% energy)
        # -------------------------
        measured_cutoff = (
        ...
        # -------------------------
        # Step 4: Proper Kalman Update
        # -------------------------

        # Prediction
        cutoff_pred = self.cutoff_est
        ...
        # Update
        cutoff_updated = cutoff_pred + K * (measured_cutoff - cutoff_pred)
        P_updated = (1 - K) * P_pred

        self.cutoff_est = cutoff_updated.detach()
        self.P = P_updated.detach()

        # -------------------------
        # Step 5: Apply Symmetric Low-Pass Mask
        # -------------------------
        final_cutoff = int(
            torch.clamp(self.cutoff_est, 1, feature_dim // 2).item()
        )

        mask = torch.zeros_like(fft_x, dtype=torch.bool)

        # Keep low positive frequencies
        mask[:, :final_cutoff] = True

        # Keep symmetric negative frequencies
        mask[:, -final_cutoff:] = True
...
class AdaptiveThresholdKalmanFFT(nn.Module):
    def __init__(
        self,
        init_tau=0.95,
        tau_min=0.85,
        ...
        # Kalman state (scalar)
        self.tau = torch.tensor(init_tau)
        self.P = torch.tensor(1e-3)

        self.Q = torch.tensor(process_var)
        self.R = torch.tensor(meas_var)

        self.tau_min = tau_min
        self.tau_max = tau_max
        self.alpha = alpha

    def spectral_entropy(self, power):
        """
        power: (B, D) power spectrum
        """
        eps = 1e-8
        prob = power / (power.sum(dim=1, keepdim=True) + eps)

        entropy = -(prob * torch.log(prob + eps)).sum(dim=1)

        # Normalize entropy to [0,1]
        max_entropy = torch.log(torch.tensor(power.shape[1], device=power.device))
    ...
    def kalman_update(self, z):
        """
        Scalar Kalman filter update for threshold
        """
        ...
        # Update
        tau_new = tau_pred + K * (z - tau_pred)
        P_new = (1 - K) * P_pred

        # Clamp for stability
        tau_new = torch.clamp(tau_new, self.tau_min, self.tau_max)

        self.tau = tau_new.detach()
        self.P = P_new.detach()

    def forward(self, x):
        """
        x: (B, D) CLIP embeddings
        """
        ...
        # 2️⃣ Power spectrum
        power = torch.abs(Xf) ** 2

        # 3️⃣ Estimate adaptive threshold measurement z
        entropy = self.spectral_entropy(power)
        z = 1.0 - self.alpha * entropy

        # Clamp measurement
        z = torch.clamp(z, self.tau_min, self.tau_max)

        # 4️⃣ Kalman update of threshold
        self.kalman_update(z)

        tau = self.tau

        # 5️⃣ Compute cumulative energy
        cumulative_energy = torch.cumsum(power, dim=1)
        total_energy = cumulative_energy[:, -1:].clone()
        energy_ratio = cumulative_energy / (total_energy + 1e-8)

        # 6️⃣ Compute cutoff index from adaptive tau
        cutoff_mask = energy_ratio >= tau
        cutoff_index = cutoff_mask.float().argmax(dim=1)

        # 7️⃣ Build symmetric frequency mask
        mask = torch.zeros_like(Xf)

        for i in range(B):
            k = cutoff_index[i].item()

            mask[i, :k+1] = 1.0
            mask[i, -k:] = 1.0  # symmetric frequencies

        # 8️⃣ Apply mask
        Xf_filtered = Xf * mask

        # 9️⃣ IFFT
        x_filtered = fft.ifft(Xf_filtered, dim=1).real
...
class StableKalmanFFT(nn.Module):
    def __init__(
        self,
        tau=0.95,              # fixed energy retention
        process_var=1e-5,      # Q (small, stable)
        init_meas_var=1e-2     # initial R
    ):
        super().__init__()
        ...
        # Kalman state: cutoff index (scalar)
        self.cutoff = None
        self.P = torch.tensor(1.0)

        # Noise terms
        self.Q = torch.tensor(process_var)
        self.R = torch.tensor(init_meas_var)

    def kalman_update(self, z):
        """
        Proper scalar Kalman update for cutoff index.
        """

        # Initialize state on first batch
        if self.cutoff is None:
            self.cutoff = z.detach()
            return self.cutoff
    ...
    def forward(self, x):
        ...
        # 3️⃣ Cumulative energy
        cumulative_energy = torch.cumsum(power, dim=1)
        total_energy = cumulative_energy[:, -1:].clone()
        energy_ratio = cumulative_energy / (total_energy + 1e-8)

        # 4️⃣ Compute batch cutoff indices using fixed tau
        cutoff_mask = energy_ratio >= self.tau
        batch_cutoff = cutoff_mask.float().argmax(dim=1).float()

        # 5️⃣ Measurement = batch mean cutoff
        z = batch_cutoff.mean()

        # 6️⃣ Adaptive measurement noise from batch variance
        batch_var = torch.var(batch_cutoff, unbiased=False)
        self.R = 0.9 * self.R + 0.1 * batch_var.detach()

        # 7️⃣ Kalman update
        cutoff_est = self.kalman_update(z)

        # Convert to integer
        k = int(torch.clamp(cutoff_est, 1, D//2).item())
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label):
        ...
        score = (score0+score1+score2+score3)/1
        if self.prompt_learner.training:
            return F.cross_entropy(logits, label)+0.5*score
